...
may use any of the other variables to predict with. You should create a report describing:
...
```{r}
# Loading data
full.trainset = read.csv("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv")
full.testset = read.csv("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv")
non.NA.columns = colMeans(is.na(full.testset)) == 0
head(non.NA.columns,10)
...
```{r}
comparison = data.frame(tree=as.vector(tree.predictions),
...
accuracy is good enough. Therefore, any of them are useful to generate the
prediction files. We opt for random forest because it is much faster in our
case.
